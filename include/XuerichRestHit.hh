#ifndef __XENON10PRESTHIT_H__
#define __XENON10PRESTHIT_H__
...
class XuerichRestHit: public G4VHit
{
public:
	XuerichRestHit();
	~XuerichRestHit();
	XuerichRestHit(const XuerichRestHit &);
	const XuerichRestHit & operator=(const XuerichRestHit &);
	G4int operator==(const XuerichRestHit &) const;
...
typedef G4THitsCollection<XuerichRestHit> XuerichRestHitsCollection;

extern G4Allocator<XuerichRestHit> XuerichRestHitAllocator;

inline void*
XuerichRestHit::operator new(size_t)
{
	return((void *) XuerichRestHitAllocator.MallocSingle());
}

inline void
XuerichRestHit::operator delete(void *pXuerichRestHit)
{
	XuerichRestHitAllocator.FreeSingle((XuerichRestHit*) pXuerichRestHit);
}
