...
main(int argc, char **argv)
{
	...
	pRunManager->SetUserInitialization(new XuerichDetectorConstruction);
	pRunManager->SetUserInitialization(new XuerichPhysicsList);

	G4VisManager* pVisManager = new G4VisExecutive;
	pVisManager->Initialize();

	// create the primary generator action
	XuerichPrimaryGeneratorAction *pPrimaryGeneratorAction = new XuerichPrimaryGeneratorAction();

	// create an analysis manager object
	XuerichAnalysisManager *pAnalysisManager = new XuerichAnalysisManager(pPrimaryGeneratorAction);
	...
	pRunManager->SetUserAction(new XuerichStackingAction(pAnalysisManager));
	pRunManager->SetUserAction(new XuerichRunAction(pAnalysisManager));
	pRunManager->SetUserAction(new XuerichEventAction(pAnalysisManager));
