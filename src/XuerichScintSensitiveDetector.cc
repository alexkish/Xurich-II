...
XuerichScintSensitiveDetector::XuerichScintSensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("ScintHitsCollection");
}

XuerichScintSensitiveDetector::~XuerichScintSensitiveDetector()
{
}

void XuerichScintSensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pScintHitsCollection = new XuerichScintHitsCollection(SensitiveDetectorName, collectionName[0]);
	...
	pHitsCollectionOfThisEvent->AddHitsCollection(iHitsCollectionID, m_pScintHitsCollection);

	m_hParticleTypes.clear();
}

G4bool XuerichScintSensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *pHistory)
{
	G4double dEnergyDeposited = pStep->GetTotalEnergyDeposit();
	G4Track *pTrack = pStep->GetTrack();

	XuerichScintHit* pHit = new XuerichScintHit();
	...
	m_pScintHitsCollection->insert(pHit);

	return true;
}

void XuerichScintSensitiveDetector::EndOfEvent(G4HCofThisEvent *pHitsCollectionOfThisEvent)
{
//  if (verboseLevel>0) { 
