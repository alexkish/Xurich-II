...
XuerichDeadXeSensitiveDetector::XuerichDeadXeSensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("DeadXeHitsCollection");
}

XuerichDeadXeSensitiveDetector::~XuerichDeadXeSensitiveDetector()
{
}

void XuerichDeadXeSensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pDeadXeHitsCollection = new XuerichDeadXeHitsCollection(SensitiveDetectorName, collectionName[0]);
	...
	pHitsCollectionOfThisEvent->AddHitsCollection(iHitsCollectionID, m_pDeadXeHitsCollection);

	m_hParticleTypes.clear();
}

G4bool XuerichDeadXeSensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *pHistory)
{
	G4double dEnergyDeposited = pStep->GetTotalEnergyDeposit();
	G4Track *pTrack = pStep->GetTrack();

	XuerichDeadXeHit* pHit = new XuerichDeadXeHit();
	...
	m_pDeadXeHitsCollection->insert(pHit);

	return true;
}

void XuerichDeadXeSensitiveDetector::EndOfEvent(G4HCofThisEvent *pHitsCollectionOfThisEvent)
{
//  if (verboseLevel>0) { 
